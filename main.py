...
async def obtener_materias_alumno(alumno_id: str):
    ...
    for inscripcion in inscripciones:
        ...
        inscripcion['materia_id'] = str(inscripcion['materia_id'])
    return inscripciones
#Metodo Put para alaumnos
@app.put("/alumnos/{alumno_id}")
async def put_alumno(alumno_id: str, alumno: Alumnos):
    result = await alumno_collection.update_one({"_id": ObjectId(alumno_id)}, {"$set": alumno.dict()})
    if result.modified_count:
        return {"message": "Alumno actualizado"}
    raise HTTPException(status_code=404, detail="Alumno no encontrado")

#Asiganr Materias a profesores
@app.post('/materias/asignar/')
async def asignar_materia_profesor(materia_id: str, profesor_id: str):
    try:
        materia_obj_id = ObjectId(materia_id)
